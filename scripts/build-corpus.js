...
async function copyFromWikipedia() {
  ...
  console.log(`Copied ${copied} articles to data/corpus/ (${missing} missing from data/wikipedia/)`);
}

async function buildManifest() {
  const files = await fs.readdir(CORPUS_DIR);
  const jsonFiles = files.filter((f) => f.endsWith('.json') && f !== 'curated-list.json');
  ...
  console.log(`Manifest written: ${MANIFEST_PATH} (${entries.length} articles)`);
}
