...
export async function hashIpAddress(ip: string, salt: string): Promise<string> {
	...
	if (!salt) {
		throw new Error(
			'[Privacy] Cannot hash IP without salt - security misconfiguration',
		);
	}
	...
	if (!hashHex || hashHex.length < 64) {
		...
		throw new Error(
			`[Privacy] Invalid hash result: expected 64 hex chars, got ${hashHex.length}`,
		);
	}
...
export async function validateIpHashing(): Promise<void> {
	...
	if (hash1 !== hash2) {
		...
		throw new Error(
			`[Privacy] Hash validation failed: same input produced different hashes`,
		);
	}
	...
	if (hash1.length !== 64) {
		...
		throw new Error(
			`[Privacy] Hash validation failed: expected 64 hex chars, got ${hash1.length}`,
		);
	}
