...
export class DocumentStore {
	...
	async storeArticle(article: WikipediaArticle): Promise<void> {
		...
		this.logger.info('Storing article in R2', {
		...
		try {
			...
			this.logger.error('Failed to store article in R2', error, {
				articleId: article.id,
			});
			throw error;
		}
	...
	async getArticle(articleId: string): Promise<WikipediaArticle | null> {
		...
		try {
			const object = await this.env.ARTICLES_BUCKET.get(
			...
			this.logger.error('Failed to retrieve article from R2', error, {
				articleId,
			});
			throw error;
		}
	...
	async deleteArticle(articleId: string): Promise<void> {
		...
		try {
			...
			this.logger.error('Failed to delete article from R2', error, {
				articleId,
			});
			throw error;
		}
	...
	async createDocument(
		doc: Omit<DocumentMetadata, 'createdAt' | 'updatedAt'>,
	): Promise<DocumentMetadata> {
		this.logger.startTimer('createDocument');
		this.logger.info('Creating document metadata', {
			documentId: doc.id,
			title: doc.title,
		});
	...
	async deleteDocument(documentId: string): Promise<void> {
		...
		try {
			await this.env.DATABASE.prepare('DELETE FROM documents WHERE id = ?')
				.bind(documentId)
				.run();

			this.logger.info('Document metadata deleted', { documentId });
		} catch (error) {
			this.logger.error('Failed to delete document metadata', error, {
				documentId,
			});
			throw error;
		}
	 ...
	 */
	async createChunks(
		chunks: Omit<TextChunk, 'createdAt'>[],
	): Promise<TextChunk[]> {
        ...
        `,
			...
			const results: TextChunk[] = chunks.map((chunk) => ({
				...chunk,
				createdAt: now,
			}));

			this.logger.endTimer('createChunks', {
				success: true,
	...
	async getChunksWithMetadata(
		...
		this.logger.debug('Getting chunks with metadata', {
	...
	async insertVectors(
		...
		try {
			await this.env.VECTOR_INDEX.upsert(
				vectors as unknown as VectorizeVector[],
			);
			this.logger.endTimer('insertVectors', {
				success: true,
	...
	async deleteVectorsByDocument(documentId: string): Promise<void> {
		...
		try {
			...
			this.logger.error('Failed to delete vectors for document', error, {
				documentId,
			});
			throw error;
		}
