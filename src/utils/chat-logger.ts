...
export class ChatLogger {
	...
	private env: Env;
	private context: Context<AppEnv>;
	private loggingEnabled: boolean;
	private logger: Logger;
	private requestId?: string;
	private trace?: TraceContext;

	constructor(env: Env, context: Context<AppEnv>) {
		this.env = env;
	...
	private async createSession(): Promise<string> {
		...
		if (
			!ipSalt ||
			ipSalt === 'default-salt' ||
			ipSalt === 'change-in-production'
		) {
			this.logger.error(
				'SECURITY: IP salt not configured or using placeholder value',
			);
			throw new Error(
				'CHAT_LOG_IP_SALT secret is not configured. Set via: wrangler secret put CHAT_LOG_IP_SALT',
			);
		}
      ...
      RETURNING id`,
		...
		if (!sessionId || typeof sessionId !== 'string') {
			throw new Error(
				`Session insert returned no ID or invalid ID type: ${sessionId}`,
			);
		}
	...
	async logMessage(params: LogChatMessageParams): Promise<void> {
        ...
        RETURNING id`,
			...
			if (!messageResult || typeof messageResult !== 'object') {
				throw new Error(
					`Message insert returned invalid result: ${messageResult}`,
				);
			}
			...
			if (!id || typeof id !== 'string') {
				throw new Error(
					`Message insert returned no ID or invalid ID type: ${id}`,
				);
			}
	...
	private async logMessageChunks(
		...
		if (failedChunks.length > 0) {
			const failureRate = (
				(failedChunks.length / sources.length) *
				100
			).toFixed(1);
