...
> {
	override async run(
		...
		const { articleId, title, content, metadata, chunkSize, chunkOverlap } =
			event.payload;
		...
		try {
			...
			const chunks = await step.do('split-text', async () => {
				...
				if (!this.env.ENABLE_TEXT_SPLITTING) {
					logger.info(
						'Text splitting disabled, using full content as single chunk',
					);
					return [
						{
							text: content,
			...
			await step.do('insert-vectors', async () => {
				logger.info('Step 6: Inserting vectors into Vectorize', {
