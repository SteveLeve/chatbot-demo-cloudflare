...
export class RAGAgent extends AIChatAgent<Cloudflare.Env, RAGAgentState> {
	override initialState: RAGAgentState = {
		traceEvents: [],
		lastSources: [],
	};
	...
	override validateStateChange(
		nextState: RAGAgentState,
		source: Connection | 'server',
	): void {
		if (source === 'server') return;

		const traceEventsMatch =
			JSON.stringify(nextState.traceEvents) ===
			JSON.stringify(this.state.traceEvents);
		const sourcesMatch =
			JSON.stringify(nextState.lastSources) ===
			JSON.stringify(this.state.lastSources);
	...
	private pushTrace(event: Omit<TraceEvent, 'traceId' | 'spanId'>): void {
		...
		const traceEvents = [...this.state.traceEvents, entry].slice(
			-MAX_TRACE_EVENTS,
		);
		this.setState({
			...this.state,
	...
	override async onChatMessage(
		...
		const spanId =
			typeof body['spanId'] === 'string' ? body['spanId'] : undefined;
		...
		const workersai = createWorkersAIModel(this.env as Env);
		...
		const retrieveTool = tool({
			...
			inputSchema: z.object({
				query: z
					...
					.max(maxQueryLength)
					.describe(
						'Search query — usually the user question or a focused sub-query',
					),
					...
					this.env as Env,
					{
						...
						onRetrieveEmpty: (q) => {
							this.pushTrace({
								type: 'guard',
								summary:
									'No corpus chunks matched — agent should refuse or ask to rephrase',
